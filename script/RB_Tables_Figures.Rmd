...
# IMPORTANTE ADD TO REST OF THE CODE
# Dataset <- Dataset %>%
...
Dataset %>%
  ...
  mutate(raw.material.type.new = recode(raw.material.type.new, Unknown = "Undetermined"),
         raw.material.type.new = fct_infreq(raw.material.type.new)) %>%
  tabyl(raw.material.type.new, Layer) %>%
  adorn_totals("col") %>%
...
### Raw material collection - Est vs. West

```{r, echo=FALSE, warning=FALSE, include=TRUE}
Dataset %>%
  filter(Class != "Pebble", raw.material.source != "Undetermined" & raw.material.source != "not.available", Layer != "A0") %>%
  tabyl(raw.material.provenience, Layer) %>%
  adorn_totals("col") %>%
  adorn_totals("row") %>%
  adorn_percentages("col") %>%
  adorn_pct_formatting(digits = 1) %>%
...
Fig3a <- Dataset %>%
  ...
  guides(fill=guide_legend(title="Provenience"))
...
# ggsave("../output/combined_plot.tiff", plot = Figure_3, width = a4_width, height = a4_height, units = "in", dpi = 300)
# 
# # Save the plot as a PDF file
ggsave("../output/combined_plot.pdf", plot = Figure_3, width = a4_width, height = a4_height, units = "in") # made in pdf and then double legend removed in Illustrator
```
  **Figure 3.**
...
# Save the plot as a high-resolution TIFF file
ggsave("../output/Figure_6.tiff", plot = Figure_6, width = 11.69, height = 4.13, units = "in", dpi = 300)
```
  *Figure 6.*
...
# Save the plot as a high-resolution TIFF file
ggsave("../output/Figure_7.tiff", plot = Figure_7, width = 11.69, height = 8.27, units = "in", dpi = 300)
...
#   geom_point(data = bipolar_cores, 
#              aes(x = log10(Volume), y = logSDI), 
#              shape = 21, fill = "orange", size = 4, stroke = 1.5, color = "black") 
# 
# plot_SDI_A2_A1 <- Dataset_cores %>%
#   filter(Layer != "A0", Core.type != "Shatter") %>%
#   ggplot(aes(x = Layer, y = logSDI)) +
...
```{r, echo=FALSE, warning=FALSE}
# Modify the Dataset_cores to lump raw.material.lumped categories and filter out "Undetermined"
Boxplot_SDI <- Dataset_cores %>%
  filter(Layer != "A0", Core.type != "Shatter", raw.material.lumped != "Undetermined") %>%
  mutate(raw.material.lumped = case_when(
    raw.material.lumped == "Local" ~ "Local",
    raw.material.lumped %in% c("Circum.local", "Distant", "Very.distant") ~ "Non-local"
  ...
  # geom_text(data = . %>%
  #             group_by(Layer) %>%
  #             mutate(is_outlier = logSDI > quantile(logSDI, 0.75) + 1.5 * IQR(logSDI) | 
  #                                 logSDI < quantile(logSDI, 0.25) - 1.5 * IQR(logSDI)) %>%
  #             filter(is_outlier),
  #           aes(label = ID), 
...
# Filter Bipolar cores
bipolar_cores <- Dataset_cores %>%
  filter(Core.classification == "Bipolar", Layer != "A0", Core.type != "Shatter")

# Add distinct shape for Bipolar cores
scatter_SDI <- scatter_SDI +
  geom_point(data = bipolar_cores, 
             aes(x = log10(Volume), y = logSDI), 
             shape = 21, fill = "blue", size = 4, stroke = 1.5, color = "black") +
  ggrepel::geom_label_repel(data = bipolar_cores, 
                   aes(x = log10(Volume), y = logSDI, label = "Bipolar"), 
                   size = 3, color = "black", fill = "white", box.padding = 0.5, point.padding = 0.5)
...
# Arrange the plots in a single figure
Figure_7 <- ggarrange(scatter_SDI, Boxplot_SDI,
                      labels = c("A", "B"), nrow = 1, common.legend = TRUE, legend = "bottom"
                      )
...
# Save the plot as a high-resolution TIFF file
ggsave("../output/Figure_7.tiff", plot = Figure_7, width = 11.69, height = 4.13, units = "in", dpi = 300)
```
